...
SELECT JOB, COUNT(*) FROM BANK_DETAIL GROUP BY JOB;

SELECT * FROM BANK_DETAIL WHERE AGE = 33;

SELECT * FROM BANK_DETAIL WHERE AGE >= 50 AND JOB = 'retired';

SELECT min(age) FROM BANK_DETAIL where JOB = 'retired';

SELECT * FROM BANK_DETAIL
WHERE EDUCATION = 'UNKNOWN' AND MARITAL = 'single'; 
...
-- 2. FIND AVG OF BALANCE :-
	SELECT AVG(BALANCE) FROM BANK_DETAIL;
-- 3. FIND MIN OF BALANCE :- 
	SELECT MIN(BALANCE) FROM BANK_DETAIL;
-- 4. FIND MAX OF BALANCE :-
	SELECT MAX(BALANCE) FROM BANK_DETAIL;

-- 5. FIND DETAIL OF CUSTOMER WHO HAVE MINIMUM BALANCE :- 
	SELECT * FROM BANK_DETAIL 
	WHERE BALANCE = (SELECT MIN(BALANCE) FROM BANK_DETAIL); 
    
-- 6. FIND DETAIL OF CUSTOMER WHO HAVE MAXIMUM BALANCE :-    
	SELECT * FROM BANK_DETAIL 
	WHERE BALANCE = (SELECT MAX(BALANCE) FROM BANK_DETAIL); 
    
-- 7. FIND LIST OF ALL THE PERSON WHI IS HAVING LOAN :-
	SELECT * FROM BANK_DETAIL
    WHERE LOAN = 'yes';
...
-- 9. FIND OUT A RECORD WITHOUT JOB WHOSE AGE IS BELOW 45 :- 
		SELECT * FROM BANK_DETAIL
		WHERE JOB = 'retired' AND AGE <= 45;
        
-- 10. FIND OUT A RECORD WHOSE BANK ACCOUNT IS HAVING A NEGATIVE BALANCE :-
		SELECT * FROM BANK_DETAIL
        WHERE BALANCE < 0;
        
-- 11. FIND OUT ALL RECORD WHO IS NOT HAVING HOUSE AT ALL ALONG WITH THERE BALACE :-
		SELECT * FROM BANK_DETAIL
        WHERE HOUSING = 'no';